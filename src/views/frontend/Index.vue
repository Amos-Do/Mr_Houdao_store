<template>
  <div>
    ...
    >
      <div class="carousel-inner vw-100">
        <div class="carousel-item active mw-100 vh-100 d-flex">
          ...
          <a href="#brand" class="down-icon">
            <font-awesome-icon icon="angle-double-down"
              class="angle-double-down"
              @click.prevent="scrollTarget('#brand')"
            />
          </a>
        </div>
        <div class="carousel-item mw-100 vh-100 d-flex">
          <a href="#brand" class="down-icon">
            <font-awesome-icon icon="angle-double-down"
              class="angle-double-down"
              @click.prevent="scrollTarget('#brand')"
    ...
    <section id="brand">
      <div
        class="container brand-info d-flex flex-wrap
        flex-md-nowrap p-3 px-md-5 pt-md-5 pb-md-3 justify-content-around
        align-items-center animated"
      >
    ...
    <section id="features">
      <div
        class="container features-info d-flex flex-wrap
        flex-md-nowrap p-3 px-md-5 py-md-3 align-items-center animated"
      >
...
export default {
  ...
  computed: {
    ...
    newProducts() {
      const vm = this;
      const allNewProducts = vm.$store.state.productsModules.products.filter(
        (item) => item.category === '最新商品',
      );
      const allLen = allNewProducts.length;
      return allNewProducts.filter(
        (item, index) => index <= allLen - 1 && index >= allLen - 3,
      );
    },
  },
  methods: {
  ...
  mounted() {
    $(window).scroll(() => {
      ...
      $('.animated').each(function animatedClass() {
