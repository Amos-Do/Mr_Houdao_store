    ...
    <table class="table mt-4 table-bordered table-hover">
      ...
      <tbody>
        <tr v-for="(item) in coupons" :key="item.id" class="align-middle">
          ...
          <td
            class="text-left align-middle text-success"
            :class="{ 'text-black-50': (!item.is_enabled) ? true : false }"
          >{{ item.code }}</td>
  ...
  methods: {
    ...
    openModal(isNew, item) {
      if (isNew) {
        ...
        const month = today.month.toString().length === 1
          ? `0${today.month}`
          : `${today.month}`;
    ...
    deletCoupon() {
      const api = `${process.env.VUE_APP_API}/api/${process.env.VUE_APP_CUSTOMPATH}/admin/coupon/${this.tempCoupon.id}`;
      const vm = this;
